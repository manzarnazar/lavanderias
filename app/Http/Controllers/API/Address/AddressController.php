...
class AddressController extends Controller
{
    ...
    {
        $hasOngoingOrders = $address->orders()
        ->whereIn('order_status', ['Pending', 'Confirm','Picked up','Processing','On Going'])
        ->exists();

        if ($hasOngoingOrders) {
            return $this->json('Address cannot be deleted because it has ongoing orders.', [], 422);
        }
        $address->delete();

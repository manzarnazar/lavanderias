                            ...
                            <table class="table table-bordered table-striped verticle-middle table-responsive-sm">
                                <tbody>
                                    ...
                                    <tr>
                                        <th>Driver photo</th>
                                        <td>
                                            <img style="max-width: 100px" src="{{ $driver->user->profile_photo_path }}"
                                                alt="">
                                        </td>
                                    </tr>
                                    <tr>
...
{{-- @dd($driverOrder->order) --}}
                                        <tr>
                                            <td class="px-2">
                                                {{ $driverOrder->order->prefix . $driverOrder->order->order_code }}
                                            </td>
                                            <td class="px-2">
                                                {{ $driverOrder->order->created_at->format('M d, Y') }}<br>
                                                <small>{{ $driverOrder->order->created_at->format('h:i a') }}</small>
                                            </td>
                                            <td class="px-2">
                                                <span style="font-size: 14px">
                                                    {{ Carbon\Carbon::parse($driverOrder->order->pick_date)->format('M d, Y') }}<br>
                                                </span>
                                                <small>
                                                    {{ $driverOrder->order->pick_hour }}
                                                    {{ $driverOrder->order->getTime(substr($driverOrder->order->pick_hour, 0, 2)) }}
                                                </small>
                                            </td>
                                            <td class="px-2">
                                                <span style="font-size: 14px">
                                                    {{ Carbon\Carbon::parse($driverOrder->order->delivery_date)->format('M d, Y') }}<br>
                                                </span>
                                                <small>
                                                    {{ $driverOrder->order->delivery_hour }}
                                                    {{ $driverOrder->order->getTime(substr($driverOrder->order->delivery_hour, 0, 2)) }}
                                                </small>
                                            </td>
                                            <td class="px-2">
                                            ...
                                            <td class="px-2 text-center text-capitalize">{{ $driverOrder->assign_for }}</td>
                                            <td class="px-2 text-center text-capitalize">{{ $driverOrder->is_approve }}</td>
                                            <td class="px-2 text-center">
                                                <a href="{{ route('order.show', $driverOrder->order->id) }}" class="btn btn-primary">
                                                    <i class="fa fa-eye"></i>
                                                </a>
                                            </td>
                                        </tr>

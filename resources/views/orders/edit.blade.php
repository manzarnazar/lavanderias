                        ...
                        <div class="card-body">
                                    ...
                                    <div class="input-group d-flex">
                                        ...
                                        <div class="flex-grow-1 customerSelect">
                                            <select name="customer_id" class="form-control select2" style="width: 100%;"
                                                ...
                                                id="customerId">
                                                <option selected value="{{ $order->customer_id }}">
                                                    {{ $order->customer->user->first_name . ' -(' . $order->customer->user->mobile . ')' }}
                                                </option>
                            ...
                            <div class="d-flex flex-column gap-3" id="basketProducts">
                                @foreach ($order->products as $product)
                                    ...
                                    <div class="d-flex gap-3 align-items-center pt-2" style="border-top: 1px dashed #eee"
                                        data-product-id="{{ $product->id }}">
                                        <div class="border rounded">
                                            <img src="{{ $product->ThumbnailPath }}" alt="" width="100"
                                                height="90" class="rounded object-fit-cover">
                                        </div>
                                        <div class="flex-grow-1">
                                            ...
                                            <div class="d-flex align-items-center gap-3">
                                                <a class="btn btn-sm m-0"
                                                    onclick="changeQuantity({{ $product->id }}, -1)">
                                                    <i class="fa fa-minus"></i>
                                                </a>
                                                <input type="text" class="text-center border rounded"
                                                    value="{{ $product->pivot->quantity }}" readonly style="width: 46px">
                                                <a class="btn btn-sm m-0" onclick="changeQuantity({{ $product->id }}, 1)">
                            ...
                            <div class="border-top mt-3 d-flex flex-column">
                                ...
                                <p
                                    ...
                                    Total Amount: <span id="totalAmount">{{ currencyPosition($order->total_amount)}}</span>
                                </p>
                                <input type="hidden" value="{{$order->total_amount}}" name="total_amount" id="totalAmountInput">
                                ...
                                <p
                                    ...
                                    Discount: <input class="form-control w-25" type="text" name="discount"
                                        id="discountInput" value="{{$order->discount}}" onkeydown="return event.key !== 'Enter';">
                                </p>

                                <p
                                    class="mb-0 border-bottom border-light py-2 d-flex justify-content-between font-weight-500">
                                    Delivery Charge: <input class="form-control w-25" type="text"
                                        name="delivery_charge" id="deliveryChargeInput" value="{{$order->delivery_charge}}" onkeydown="return event.key !== 'Enter';">
                                </p>
                                @php
                                    $grandtotal = $order->total_amount+$order->delivery_charge-$order->discount;
                                @endphp


                                <h3 class="mb-0 border-bottom border-light py-2 d-flex justify-content-between">
                                    Grand Total: <span id="grandTotal">{{currencyPosition($grandtotal)}}</span>
                                </h3>
                                <input type="hidden" value="{{ $grandtotal }}" name="grand_total" id="grandTotalInput">
            ...
            @foreach ($order->products as $product)
                {
                    ...
                    name: '{{ $product->name }}',
                    image_path: "{{ $product->thumbnailPath }}",
                },
            @endforeach
        ];
